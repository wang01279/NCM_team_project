  ...
  return (
    <div className="container py-2">
      <div className="row g-2 mb-3">
        <div className="col-12 col-md-3">
          <div className="input-group">
            ...
            <input type="text" className="form-control" placeholder="搜尋商品..." />
          </div>
        </div>
        <div className="col-12 col-md-3">
      ...
      <div className={`filter-panel ${isFilterOpen ? 'show' : ''}`}>
        <button className="btn-close position-absolute top-0 end-0 m-3" aria-label="Close" onClick={closeFilterPanel}></button>
        ...
        <div className="mb-4">
          ...
          <div className="text-center mt-2">
            價格：NT${Math.min(minPrice, maxPrice)} ~ NT${Math.max(minPrice, maxPrice)}
          </div>
        </div>
            ...
            {[
              ...
              <button
                ...
                className={`btn btn-outline-primary filter-btn ${
                  selectedFilters.material.includes(option.value) ? 'active' : ''
                }`}
          ...
          <div className="filter-button-group">
            {[
              ...
              <button
                ...
                type="button"
                className={`btn btn-outline-primary filter-btn ${
                  selectedFilters.function.includes(option.value) ? 'active' : ''
                }`}
                onClick={() => toggleFilter(option.type, option.value)}
              >
                {option.label}
      ...
      <div className={`overlay ${isFilterOpen ? 'show' : ''}`} onClick={closeFilterPanel}></div>
    </div>
  );
};

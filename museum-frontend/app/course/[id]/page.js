...
export default function CourseDetailPage() {
  ...
  const courseData = useMemo(() => {
    ...
    return {
      ...
      materialsIncluded: state.course.materials_included ? '含材料與工具' : '不含材料',
      artist: state.artist,
    ...
    <>
      ...
      <section className="course-gallery container py-4">
        <div className="row g-3">
          {Array.isArray(courseData.images) && courseData.images.slice(0, 3).map((img, idx) => (
    ...
    <section className="content-section" data-aos="fade-up">
      ...
      <div className="instructor-card">
        ...
        <div className="instructor-info">
          ...
          <p>{artist.bio || '從事陶藝創作超過30年，專精於傳統陶藝技法的傳承與創新。作品多次在國際展覽中展出，並獲得多項重要獎項。'}</p>
          <ul className="experience-list">
            {Array.isArray(artist.experiences) && artist.experiences.length > 0 ? (
              artist.experiences.map((exp, index) => (
                <li key={index}>{exp}</li>
              ))
            ) : (
              <li>尚無經歷資料</li>
